...
const UserDashboard = ({chartRef,campaigns,activity,handleclick,toggleDropdown,isOpen,categories,handleCheckboxChange}) => {
  return (
    <>
                ...
                <tbody className="bg-white dark:bg-gray-800">
                  {campaigns.map((campaign, index) => (
                             ...
                             <td className="p-4 whitespace-nowrap">
                              {campaign.status == 'Verfied' ? <span className="bg-green-100 text-green-800 text-xs font-medium mr-2 px-2.5 py-0.5 rounded-md">Completed</span>
                              :  <span className="bg-orange-100 text-orange-800 text-xs mr-2 font-medium px-2.5 py-0.5 rounded-md">Pending</span>}
                             </td>
                             <td>
                              <Link to={`../campaign-detail/${campaign.id}`} className="text-blue-700 bg-whit hover:text-blue-900 font-medium text-sm px-10 py-2.5 text-center inline-flex items-center">
                              More Details
