    ...
    <Card className="bg-white shadow-lg rounded-lg relative">
      ...
      <div className="absolute top-4 right-2 mr-3">
        <span className={`px-4 py-1 rounded-lg text-sm font-semibold ${
          ...
          campaign.status === 1 ? 'Approved' : 'Suspended'}
        </span>
      </div>
      ...
      {showRejectReason && (
        <div className="mt-4 p-4 bg-red-100 rounded-lg">
          <h3 className="text-lg font-medium text-red-800">Provide Reason for Suspension</h3>
          <textarea
            value={rejectReason}
            onChange={(e) => setRejectReason(e.target.value)}
            className="w-full p-2 mt-2 border border-red-300 rounded-md"
            rows="4"
            placeholder="Please write the reason for suspension here..."
          ></textarea>
